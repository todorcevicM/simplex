...
def simplex(A, b, c, max = True):
    ...
    # postavljanje base matrice 

    no_iterations = 0
    tested_base = np.array([])
    # 2. obrada: 
    ...
    while True:
        ...
        if no_iterations > 20:
            # novi base
            base_variables = np.where(base == 1)
            base_variables += np.array([1] * len(base_variables))
            tested_base = np.append(tested_base, base_variables)

            no_base_variables = len(base_variables[0])
            no_tuples = int(len(tested_base) / no_base_variables)
            new_base_indexes = np.array([]) 
            for i in range(no_tuples): 
                to_test = np.array([])
                for j in range(no_base_variables): to_test = np.append(to_test, tested_base[i * no_base_variables + j])
                to_test = list(to_test)
                for i, v in enumerate(to_test): to_test[i] = int(v)

                flag = False
                for tuple_t in combinations(range(1, 1 + c.shape[0]), no_base_variables):
                    t = list(tuple_t)
                    for i, v in enumerate(t): t[i] = int(v)
                    if t == to_test: 
                        flag = True
                        break

            if flag == False:
                new_base_indexes = np.append(new_base_indexes, t)
                tested_base = np.append(tested_base, t)
                print(f"new_base_indexes: {new_base_indexes}")

                B = [A[:, i] for i in new_base_indexes]
                try:
                    B_inv = np.linalg.inv(B)
        ...
        max_index = np.argmax(finding_min)
        if no_iterations > 0:
            if max: 
                ...
                if finding_min[max_index] < 0:
                    ...
                    for (index, i) in enumerate(base):
                        if i == 1:
                            ...
                            print(f"\tPromenljiva x_{index + 1} = 0")
                    print(f"Tabela na kraju obrade: \n{table}")
                    return 
        ...
        if max:
            ...
            variable_to_replace = int(indexes[max_index])
        ...
        if max:
            ...
            table[0, -1] = finding_min[max_index]
